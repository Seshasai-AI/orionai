...
// ── Avatar ─────────────────────────────────────────────────────────────────
const AV = `<svg viewBox="0 0 28 28" fill="none">
  <circle cx="14" cy="4"  r="2.5" fill="white"/>
  <circle cx="22" cy="9"  r="1.8" fill="white" opacity=".7"/>
  <circle cx="6"  cy="9"  r="1.8" fill="white" opacity=".7"/>
  <circle cx="14" cy="14" r="3"   fill="white"/>
  <circle cx="21" cy="20" r="1.8" fill="white" opacity=".6"/>
  <circle cx="7"  cy="20" r="1.8" fill="white" opacity=".6"/>
</svg>`;
...
function showTyping() {
  ...
  r.id = 'typing'; r.className = 'typing-row';
...
// ── Web search keywords ────────────────────────────────────────────────────
const SPORTS = ['ipl','cricket','match','winner','score','wicket','t20','football','goal','result','world cup'];
const SEARCH = ['today','latest','current','now','recent','news','price','weather','update','live','stock','2025','2026'];
function needsSearch(q) {
  const lq = q.toLowerCase();
  return SPORTS.some(k => lq.includes(k)) || SEARCH.some(k => lq.includes(k));
}
...
async function send(text) {
  ...
  userRow.innerHTML = `<div class="bubble">${esc(text)}</div>`;
  feedEl.appendChild(userRow);
  scrollBottom();

  showTyping();
  const willSearch = needsSearch(text);
  ...
  try {
    ...
    av.className = 'ai-av'; av.innerHTML = AV;
    ...
    feedEl.appendChild(row); scrollBottom();
    ...
    if (willSearch) {
      const b = bub.querySelector('.web-badge');
      if (b) b.innerHTML = b.innerHTML.replace('Searching web…', 'Web search used');
    }
    ...
    const av = document.createElement('div'); av.className = 'ai-av'; av.innerHTML = AV;
    const bub = document.createElement('div'); bub.className = 'bubble';
    bub.innerHTML = `<p style="color:#f87171">❌ ${err.message || 'Something went wrong.'}</p>`;
    row.appendChild(av); row.appendChild(bub); feedEl.appendChild(row);
...
function renderHistory() {
  ...
  sessions.forEach(s => {
    ...
    b.className = 'history-item' + (s.id === currentId ? ' active' : '');
...
function loadSession(id) {
  ...
  messages.forEach(m => {
    if (m.role === 'user') {
      ...
      r.innerHTML = `<div class="bubble">${esc(m.content)}</div>`;
      feedEl.appendChild(r);
    } else {
      const r = document.createElement('div'); r.className = 'row assistant';
      const av = document.createElement('div'); av.className = 'ai-av'; av.innerHTML = AV;
      const b = document.createElement('div'); b.className = 'bubble';
      b.innerHTML = md(m.content);
      r.appendChild(av); r.appendChild(b); feedEl.appendChild(r);
    }
  });
  renderHistory(); closeSidebar(); scrollBottom();
}
...
// ── Sidebar ────────────────────────────────────────────────────────────────
function openSidebar()  { sidebarEl.classList.add('open'); overlayEl.classList.add('on'); }
function closeSidebar() { sidebarEl.classList.remove('open'); overlayEl.classList.remove('on'); }

// ── Events ─────────────────────────────────────────────────────────────────
sendEl.onclick = () => {
  const t = inputEl.value.trim(); if (!t) return;
  inputEl.value = ''; resize(); send(t);
};
...
document.querySelectorAll('.chip').forEach(c => {
  c.onclick = () => { inputEl.value = c.dataset.p; resize(); sendEl.disabled = false; sendEl.click(); };
});
...
(async function init() {
  try {
    ...
    const mdata = await mr.json(), hdata = await hr.json();
    modelEl.textContent = mdata.current || 'OrionAI';
    if (hdata.web_search_ready && webRowEl) webRowEl.style.display = 'flex';
